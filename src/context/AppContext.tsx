...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Auth: current signed-up user (persisted in localStorage)
  const [user, setUser] = useState<AuthUser | null>(() =>
    loadFromLocalStorage<AuthUser | null>(STORAGE_KEYS.USER, null)
  );
  ...
  const signupUser = (name: string, email: string) => {
    ...
    setUser(newUser);
    // Update the profile so the dashboard/header shows the signed-up name
    setSettings((prev) => ({
      ...prev,
      studentName: newUser.name,
      degreeName: prev.degreeName || 'BS Artificial Intelligence'
    }));
    localStorage.setItem('ai_hub_goals_customized', 'true');
    setActiveTab('dashboard');
    showToast(`Welcome aboard, ${newUser.name}! 🎉`);
    triggerConfetti();
  };
